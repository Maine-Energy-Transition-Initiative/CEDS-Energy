...
Animation1 <- ggplot(Carriage_heating_and_electricity, mapping = aes(x = Date, y = BTU, color = Fuel_Type, group = Fuel_Type)) +
  geom_line() + 
 # geom_line(Carriage_heating_oil, mapping = aes(x = `Delivery Date`, y = BTU), color = "blue") + 
  geom_vline(xintercept=as.POSIXct(as.Date("2021-08-01")),
                linetype=1, colour="black") +
   annotate("text", x=as.POSIXct(as.Date("2021-06-01")), y = 5e+06, label="David started as the Director of Energy", angle=90) +
