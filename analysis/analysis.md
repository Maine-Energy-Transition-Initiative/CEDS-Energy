...
Cranberry2023Clean <- Cranberry2023 %>%
  mutate(High_Temp = gsub(" °F", "", High_Temp)) %>%
  mutate(Avg_Temp = gsub(" °F", "", Avg_Temp)) %>%
  mutate(Low_Temp = gsub(" °F", "", Low_Temp)) %>%
  mutate(High_DP = gsub(" °F", "", High_DP)) %>%
  mutate(Avg_DP = gsub(" °F", "", Avg_DP)) %>%
  mutate(Low_DP = gsub(" °F", "", Low_DP)) %>%
  mutate(High_Humid = gsub(" %", "", High_Humid)) %>%
  mutate(Avg_Humid = gsub(" %", "", Avg_Humid)) %>%
  mutate(Low_Humid = gsub(" %", "", Low_Humid)) %>%
  mutate(High_Speed = gsub(" mph", "", High_Speed)) %>%
  mutate(Avg_Speed = gsub(" mph", "", Avg_Speed)) %>%
  mutate(Low_Speed = gsub(" mph", "", Low_Speed)) %>%
  mutate(High_Pressure = gsub(" in", "", High_Pressure)) %>%
  mutate(Low_Pressure = gsub(" in", "", Low_Pressure)) %>%
  mutate(Precip_Accum = gsub(" in", "", Precip_Accum)) 
  # mutate_at(c(Cranberry2023$High_Temp:Cranberry2023$Precip_Accum), as.numeric)
```
