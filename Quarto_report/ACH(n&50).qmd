...
ggplot(Building_Audit, aes(y=1))+
  geom_bar(width=0.5, fill ="#EDEDED")+ #add8e6
  #scale_fill_gradient(low="#FF8888",high="#FF0000")+ #it should be working idk why it doesn't
   geom_vline(xintercept=0.98, #move this to before previous x intercepts
                linetype=1, colour="#1B9E77", size= 3)+ #(maybe have coditional color? red if ACHn is to big blue if it's ok)
  #scale_x_reverse()+
  geom_vline(xintercept= c(0, 0.04, 0.35, 0.41, 0.6,1), size= 1.3, ylim)+
 
  
#The vline text lables
  geom_text(aes(x=0.98, label="Your Home", y=1.15), nudge_x= -0.03, colour="#1B9E77", angle=90, size = 5, ) + #move this to the legend
...
#The vline text values 
  geom_text(aes(x=0.98, label=0.98, y=0.85), nudge_x= -0.03, colour="#1B9E77", angle=90, size = 5) + #move this to the legend
