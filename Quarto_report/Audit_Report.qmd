...
#select all columns that have information about interior/living spaces
  select(wall_stud_insulation_type, 
         ...
         which_appliance_did_you_measure_1, which_appliance_did_you_measure_2
         )|>
...
mutate(stock_framing = case_when(
  ...
  framing_type == "Platform" ~ 'The house has platform framing, which means that wall cavities do not extend the full height of the building, but are instead separated by floors. ', TRUE ~ 'NA'
  ))|>
...
# rename the columns as they need to be displayed
  mutate(Info = recode(Info,  
                       ...
                       general_comment_about_the_bathroom_s = 'Bathroom(s):'
                       ))|>
...
```{r}
#| label: blower door ACHnatural diagram with basement door open
#| eval: ACHnaturalBasement != NA
#| echo: false
#| output: false
...
ggplot(ACHnatural_ggplot, aes(y = 1)) +
  ...
  # Your home's ACHn value line
  geom_vline(xintercept = ACHnaturalBasement, 
             linetype = 1, colour = "#1B9E77", size = 3) +
  ...
  # Labels for "Your Home" and value
  geom_text(aes(x = ACHnaturalBasement, label = "Basement Open", y = 1.15), 
            nudge_x = -0.03, colour = "#1B9E77", angle = 90, size = 5) +
  geom_text(aes(x = ACHnaturalBasement, label = round(ACHnaturalBasement, 2), y = 0.85), 
            nudge_x = -0.03, colour = "#1B9E77", angle = 90, size = 5) +
...
# Save as image
ggsave("achnatural_ggplot1_basement.png", width = 15, height = 5)

```
![](achnatural_ggplot1_basement.png)
```{r}
#|label: ACHNatural basement diagram caption
#|eval: ACHNaturalBasement != NA
#|echo: false
#|output: false

cat("This is the ACH Natural value for your home while the interior basement door is open. ")
if(ACHnaturalBasement/ACHnatural > 0.9){
  cat("This number is significantly higher than the home's ACHNatural, indicating that the basement is poorly sealed. This is not ideal, as this is allowing moisture and unconditioned air from the outside to freely enter the basement, where it can then travel into the living space through other air leaks in the home.")
}else{
  cat("This number is fairly close to the home's ACHNatural, which could either be the result of your basement being well air sealed, or there being significant air transfer between the basement and the living space.")
  if(clean_audit_data$basement_crawlspace_insulation_type == "Uninsulated"){
    cat("Since your basement is uninsulated, it is also most likely not air sealed, so the latter scenario is more plausible.")
  }else{cat("")}
}
```
