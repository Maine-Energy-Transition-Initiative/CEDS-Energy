...
energy_test_googlesheets <- energy_test_googlesheets |>
  ...
  mutate(combustionst1 = str_detect(`Could you perform combustion safety tests in worst case scenario? 1`, pattern = "Yes"))
...
`r if(energy_test_googlesheets$combustion_fuels == TRUE){"We assessed combustion appliances that burn fossil fuels such as propane, heating oil, or kerosene. These include furnaces, boilers, water heaters, and gas ovens."}else{""}` **DELETE UNWANTED** We visually inspected the combustion appliance(s) in your home, as well as conducted combustion safety tests. This included measuring for carbon monoxide and testing that flue gases are properly exhausting from the home. but we were unable to perform combustion safety tests.

```{r}
#| label: If else chapter 1.5 not working options
#| echo: false
#| eval: false
#| warning: false


# all below are not working
...
`r if(energy_test_googlesheets$combustion_fuels == TRUE){"We visually inspected the combustion appliance(s) in your home,"}else{""}`

`r if(energy_test_googlesheets$combustionst1 == TRUE){"as well as conducted combustion safety tests. This included measuring for carbon monoxide and testing that flue gases are properly exhausting from the home."}else{""}`

`r if(energy_test_googlesheets$combustionst1 == FALSE){"but we were unable to perform combustion safety tests."}else{""}`
...
```{r}
#| label: setup data 4.0
#| eval: false
#| echo: false
recommendations_4.0 <- ranked_recommendations|>
   filter(Rank != 0)#|>
   #select(-Rank)
